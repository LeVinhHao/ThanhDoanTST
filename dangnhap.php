                    ...
                    if(!empty($u)&&!empty($p)){
                        if( mysqli_query($conn,$query)->fetch_assoc()==true){
                            if( $admin==$result['username'] && $p==$result['password']){ 
                                header("location:http://localhost:8080/ThanhDoanTST/admin.php");                     
                            }
                            else if($result['username']==$u&& $result['password']==$p){ 
                                $_SESSION ['login']['idkh']= $result['id'];
                                header("location:http://localhost:8080/ThanhDoanTST/TrangChu.php");
                            }else{
...
<div id="wrapper">
        <form method="POST" action="" id="form-login">
            ...
            <a href="http://localhost:8080/ThanhDoanTST/dangky.php" style="color:white; text-decoration:none; display:flex; justify-content:center;">
